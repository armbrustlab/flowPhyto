...
.processRawData <- function(cruise.dir='.', steps = 1:3, clust.concat.ct=3, filter.width=1.5, filter.notch=1, filter.edge=1, filter.slope=NA, classify.varnames=CHANNEL.CLMNS.SM, classify.func= 2, classify.numc=0, classify.noise=0, map.margin=2, output.path='.', log.dir='.', def.path=NULL, debug=FALSE, fresh=TRUE, cleanup=TRUE, pct=.97, parallel=FALSE, submit.cmd='qsub', range=NULL){
  ...
  if(any(steps %in% c(1,2,3))){
    ...
    add.opts <- paste('--map.margin=', map.margin,' --concat.ct=', clust.concat.ct,' --def.path=', def.path,' --width=', filter.width,' --notch=', filter.notch, ' --slope=', filter.slope, ' --edge=', filter.edge, ' --func=',classify.func, ' --varnames=', paste(classify.varnames, collapse=','), ' --numc=', classify.numc, ' --noise=', classify.noise, sep='')
